...
# SQL Veritabanı
DB_URI = os.environ.get("DATABASE_URL", "sqlite:///herlock.db")
...
ASISTAN = 2147434581 # Bot yardımcısı
...
async def autobot():
    ...
    if usnm:
        ...
        username = "herlock_" + (str(uid))[5:] + "_bot"
    bf = "@BotFather"
    ...
    if not isdone.startswith("Good."):
        await bot.send_message(bf, "My Herlock Bot")
        time.sleep(3)
        isdone = (await bot.get_messages(bf, limit=1))[0].text
        if not isdone.startswith("Good."):
            LOGS.info(
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "herlock_" + (str(uid))[6:] + str(ran) + "_bot"
        await bot.send_message(bf, username)
        time.sleep(3)
        ...
        if nowdone.startswith("Done!"):
            ...
            await bot.send_message(bf, f"@{username}")
            time.sleep(3)
            await bot.send_file(bf, "image/herlock.jpg") 
            heroku_var["BOT_TOKEN"] = token
            heroku_var["BOT_USERNAME"] = username
            LOGS.info(f"HerlockUserBot ile otomatik olarak asistan oluşturuldu. @{username}")
        else:
            LOGS.info(
